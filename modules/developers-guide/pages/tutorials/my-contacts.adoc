...
* フロントエンド開発のために `+node.js+` がインストールされており、プロジェクトで `+npm install+` を使用してパッケージをインストールすることができます。
ローカルのオペレーティングシステムやパッケージマネージャに node をインストールする方法については、 link:https://nodejs.org/en/[Node] のウェブサイトを参照してください。

* link:.../.../quickstart/local-quickstart{outfilesuffix}#download-and-install[ダウンロードとインストール] で説明されている通り、 {sdk-short-name} パッケージをダウンロードしてインストールします。

* IDE として Visual Studio Code を使用している場合、 link:./../quickstart/local-quickstart{outfilesuffix}#install-vscode[language editor plug-inのインストール] で説明されているように、 {proglang} の Visual Studio Code プラグインがインストールされています。

* ローカルコンピュータ上で実行されているローカルキャニスターの実行環境プロセスを停止します。
...
React を使ったことがない方は、フロントエンドのコードを編集する前に、 link:https://reactjs.org/tutorial/tutorial.html[React 紹介] チュートリアルや、 link:https://reactjs.org/[React ウエブサイト] をご覧になるとよいでしょう。
...
- 新しい連絡先を追加した後に入力フィールドをクリアするように、フロントエンドのコードを変更します。例えば、 `+onClick+` イベントの一部として。
- プログラムの {proglang} 関数を変更して、 `+Name+` フィールドで正確な文字列マッチングではなく、部分的な文字列マッチングを行うようにしました。(変更内容をローカル環境でテストするには、 `dfx deploy` を実行する必要があります)
- 異なるフィールドに基づいて検索できるように {proglang} プログラムを変更します。
