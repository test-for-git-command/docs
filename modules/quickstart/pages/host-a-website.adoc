...
始める前に、私たちの {sdk-short-name} (DFX) と Cycle ウォレットがセットアップされていることを確認してください。 Cycle ウォレットをセットアップするには、xref:quickstart:cycles-faucet.adoc[Fauset クイックスタート] を見るか、または ICP を購入して xref:quickstart:network-quickstart.adoc[Network Deployment] ガイドに従ってください。
